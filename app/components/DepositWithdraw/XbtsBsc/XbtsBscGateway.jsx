...
import XbtsBscGatewayDepositRequest from "./XbtsBscGatewayDepositRequest";
...
class XbtsBscGateway extends React.Component {
    constructor(props) {
        super();

        this.state = {
            activeCoin: this._getActiveCoin(props, {action: "deposit"}),
            action: props.viewSettings.get("bscAction", "deposit")
        };
    }

    _getActiveCoin(props, state) {
        let cachedCoin = props.viewSettings.get(
            `activeCoin_bsc_${state.action}`,
            null
        );
    ...
    onSelectCoin(e) {
        ...
        let setting = {};
        setting[`activeCoin_bsc_${this.state.action}`] = e.target.value;
    ...
    changeAction(type) {
        ...
        SettingsActions.changeViewSetting({["bscAction"]: type});
    }
...
export default connect(XbtsBscGateway, {
    listenTo() {
