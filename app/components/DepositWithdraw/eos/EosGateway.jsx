...
import EosGatewayDepositRequest from "./EosGatewayDepositRequest";
...
class EosGateway extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            action: props.viewSettings.get("eosAction", "deposit")
        };
    }

    _getActiveCoin(props, state) {
        let cachedCoin = props.viewSettings.get(
            `activeCoin_eos_${state.action}`,
            null
        );
    ...
    onSelectCoin(e) {
        ...
        let setting = {};
        setting[`activeCoin_eos_${this.state.action}`] = e.target.value;
    ...
    changeAction(type) {
        ...
        SettingsActions.changeViewSetting({["eosAction"]: type});
    }
...
export default connect(EosGateway, {
    listenTo() {
