...
import EthGatewayDepositRequest from "./EthGatewayDepositRequest";
...
class EthGateway extends React.Component {
    constructor(props) {
        super();

        this.state = {
            activeCoin: this._getActiveCoin(props, {action: "deposit"}),
            action: props.viewSettings.get("ethAction", "deposit")
        };
    }

    _getActiveCoin(props, state) {
        let cachedCoin = props.viewSettings.get(
            `activeCoin_eth_${state.action}`,
            null
        );
    ...
    onSelectCoin(e) {
        ...
        let setting = {};
        setting[`activeCoin_eth_${this.state.action}`] = e.target.value;
    ...
    changeAction(type) {
        ...
        SettingsActions.changeViewSetting({["ethAction"]: type});
    }
...
export default connect(EthGateway, {
    listenTo() {
