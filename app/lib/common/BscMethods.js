...
export function fetchCoinList(url = bscAPIs.BASE + bscAPIs.COINS_LIST) {
    return fetch(url, {method: "post"})
        ...
        .catch(err => {
            console.log("error fetching bsc list of coins", err, url);
        });
}
...
export function validateAddress({url = bscAPIs.BASE, walletType, newAddress}) {
...
function hasWithdrawalAddress(wallet) {
    return bscStorage.has(`history_address_${wallet}`);
}

function setWithdrawalAddresses({wallet, addresses}) {
    bscStorage.set(`history_address_${wallet}`, addresses);
}

function getWithdrawalAddresses(wallet) {
    return bscStorage.get(`history_address_${wallet}`, []);
}

function setLastWithdrawalAddress({wallet, address}) {
    bscStorage.set(`history_address_last_${wallet}`, address);
}

function getLastWithdrawalAddress(wallet) {
    return bscStorage.get(`history_address_last_${wallet}`, "");
}
